...
describe("order/sortSubgraph", () => {
    ...
    beforeEach(() => {
        ...
        ["0", "1", "2", "3", "4"].forEach((v, i) => g.setNode(v, {order: i}));
    ...
    it("sorts a flat subgraph based on barycenter", () => {
        ...
        expect(sortSubgraph(g, "movable", constraintGraph).vs).toEqual(["y", "x"]);
    });
    ...
    it("preserves the pos of a node (y) w/o neighbors in a flat subgraph", () => {
        ...
        expect(sortSubgraph(g, "movable", constraintGraph).vs).toEqual(["z", "y", "x"]);
    });
    ...
    it("biases to the left without reverse bias", () => {
        ...
        expect(sortSubgraph(g, "movable", constraintGraph).vs).toEqual(["x", "y"]);
    });
    ...
    it("biases to the right with reverse bias", () => {
        ...
        expect(sortSubgraph(g, "movable", constraintGraph, true).vs).toEqual(["y", "x"]);
    });
    ...
    it("aggregates stats about the subgraph", () => {
        ...
        const results = sortSubgraph(g, "movable", constraintGraph);
    ...
    it("can sort a nested subgraph with no barycenter", () => {
        ...
        expect(sortSubgraph(g, "movable", constraintGraph).vs).toEqual(["x", "z", "a", "b", "c"]);
    });
    ...
    it("can sort a nested subgraph with a barycenter", () => {
        ...
        expect(sortSubgraph(g, "movable", constraintGraph).vs).toEqual(["x", "a", "b", "c", "z"]);
    });
    ...
    it("sorts border nodes to the extremes of the subgraph", () => {
        ...
        expect(sortSubgraph(g, "sg1", constraintGraph).vs).toEqual(["bl", "x", "y", "z", "br"]);
    });
    ...
    it("assigns a barycenter to a subgraph based on previous border nodes", () => {
        ...
        expect(sortSubgraph(g, "sg", constraintGraph)).toEqual({
            barycenter: 0.5,
            weight: 2,
            vs: ["bl2", "br2"]
        });
